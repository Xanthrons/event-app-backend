...
exports.register = async (req, res) => {
  try {
    ...
    if (!role || !["individual", "business", "admin"].includes(role)) {
      return res.status(400).json({ message: "Invalid role provided!" });
    }

    let existingUser;

    // Check uniqueness of email based on role
    if (role === "business") {
      existingUser = await Business.findOne({
        business_email: userData.business_email,
      });
    } else if (role === "individual") {
      existingUser = await Individual.findOne({ email: userData.email });
    } else if (role === "admin") {
      existingUser = await Admin.findOne({ email: userData.email });
    }
    ...
    if (role === "business") {
      newUser = new Business({ ...userData, password: hashedPassword });
    } else if (role === "individual") {
      newUser = new Individual({ ...userData, password: hashedPassword });
    } else if (role === "admin") {
      newUser = new Admin({ ...userData, password: hashedPassword });
    }
...
exports.login = async (req, res) => {
  try {
    ...
    const individualUser = await Individual.findOne({ email }).select(
      "+password"
    );
...
exports.googleAuth = async (req, res) => {
  try {
    ...
    if (!googleEmail) {
      return res
        .status(400)
        .json({ message: "Could not retrieve email from Google token." });
    }
    ...
    if (user) {
      ...
      return res.status(200).json({
        ...
        user: {
          ...
          role: user.role,
          email: user.email || user.business_email,
          profile_picture: user.profile_picture || googlePicture,
          first_name: user.first_name || googleName,
        },
      });
    } else {
      // Create a new user
...
exports.verifyResetCode = async (req, res) => {
  try {
    ...
    user = await Individual.findOne({ email }).select(
      "+passwordResetCode +passwordResetCodeExpires"
    );
    if (!user) {
      user = await Business.findOne({ business_email: email }).select(
        "+passwordResetCode +passwordResetCodeExpires"
      );
    }
    if (!user) {
      user = await Admin.findOne({ email }).select(
        "+passwordResetCode +passwordResetCodeExpires"
      );
    }
    ...
    res
      .status(200)
      .json({ message: "Verification code is valid.", tempToken: tempToken });
  } catch (error) {
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        message: "Invalid or expired temporary token for password reset.",
      });
    }
...
exports.testDeleteAccountByEmail = async (req, res) => {
  try {
    ...
    if (role === "individual") {
      ...
      deletionResult = await Business.findOneAndDelete({
        business_email: email,
      });
    } else if (role === "admin") {
    ...
    if (!deletionResult) {
      return res
        .status(404)
        .json({ message: "Account with provided email not found." });
    }

    res
      .status(200)
      .json({ message: "Account deleted successfully (TEST FUNCTION)." });
  } catch (error) {
...
exports.uploadProfilePicture = async (req, res) => {
  try {
    ...
    if (userRole === "individual") {
      ...
      return res
        .status(400)
        .json({ message: "Invalid user role for profile picture upload." });
    }
